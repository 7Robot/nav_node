...
class NavigationNode():
    ...
    def __init__(self, avoidance_mode, avoidance_trigger_distance,
        ...
        #np.save("cvmap.npy", self.cvmap)
        self.cvmap = np.load("cvmap.npy")
    ...
    def find_path_without_obstacles(self, start_pos, end_pos):
        """
        Trouve le chemin le plus court entre les deux positions
        sans prendre en compte les collisions avec les obstacles mouvants
        """
    ...
    def adapt_path(self, new_obstacles, distance=3):
        """
        distance: number of cells to check

        create a new path if there is an obstacle in the current path
        """
        #  Check if there is an obstacle in the path
        first_impacted_point = (None, None)

        if self.path!= None and len(self.path)>0:
            print("Obstacle on path !!!")
            for e in new_obstacles:
                if e in self.path[0:distance]:
                    # Obstacle in the path
                    for neighbour in self.get_neighbor(e):
                        self.t[e] = 'OPEN'
                        self.OPEN.add(e)
                        if first_impacted_point[0] == None:
                            first_impacted_point = e
            
            # Recompute path
            s = first_impacted_point
            while s != self.position_goal:
                if s in self.obstacles:
                    # Modify cost
                    if self.t[s] == "CLOSED":
                        self.insert_dstar(s, self.h[self.PARENT[s]]+self.cost(s, self.PARENT[s]))
            ...
            self.rebuild_path((self.robot_data.position.x*100//self.discretisation, self.robot_data.position.y*100//self.discretisation), self.position_goal)
    ...
    def robot_data_callback(self, msg):
        ...
        # Update Path
        if self.path != None and len(self.path) > 0:
            pos = np.array([msg.position.x*100/self.discretisation, msg.position.y*100/self.discretisation])
            point_path = np.array(self.path[0])
            dist =np.linalg.norm(pos-point_path) 
            if dist<1:
                print("Currently at "+str(pos)+", arrived at point " + str(self.path.pop(0)))
                print("Path is now : " + str(self.path))   
    ...
    def obstacles_callback(self, msg):
        ...
        if new_obstacles != []:
            ...
            # Adapt path
            self.adapt_path(dyn_without_duplicate)
